...
class ProductController extends Controller
{
    public function productcatagory(Request $request)
  {
   ...
   public function insertpc(Request $request)
    ...
    //   print_r($search);
    //   die();
      return view('Admin.showpc')->with(['data'=>$data,"search"=>$search]);
    }

   public function editcustomer(Request $request,$id)
  {
      $data=DB::table('product')->where('id',$id)->first();
      return view ('Admin.editpc')->with('data',$data);
  }
        ...
        if($files=$request->file('img'))
        {
          foreach($files as $file)
          {
            ...
            if($img [1] =='png' ||  $img [1] == "jpg" ||  $img [1] =="jpeg"||  $img [1] == "mp4" )
            {
              ...
              DB::table('product')->where('id',$data['id'])->update(["brand"=>$data['brand'],"tittle"=>$data['title'],"descripation"=>$data['descripation'],"price"=>$data['price'],"item_img"=>$t,'updated_at'=>$time]);
...
//      return view('Admin.color',compact('products','selected_id'));
...
return view('Admin.color')->with(['p'=>$p]);
}
}
